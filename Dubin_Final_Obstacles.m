...
source_path = [];
for i = 1:m;
    source_path = [source_path; route(row(i):row(i)+1,:)];
end

goal_path = [];
for i = 1:n;
    goal_path = [goal_path; route1(row1(i):row1(i)+1,:)];
end


%%%% Algorithm for connecting Path %%%%
total_path = [];
% for k = 1:min(m,n);
    if (m==n)
        for i = 1:2:size(goal_path);
            dist_diff = sqrt(sum(goal_path(i:i+1,end)-source_path(i:i+1,end)).^2);   % calculate distance from source/goal path
            if (dist_diff <= delta)
                path = [source_path fliplr(goal_path)];
                hold on; plot(gca, path(1,:), path(2,:), '--b','Linewidth',1); hold on
                total_path = [total_path; path];
%                 hold on; plot(gca, total_path(i:i+1,:), total_path(i:i+1,:), '--b','Linewidth',1); hold on
            end
        end
    else if (m > n)
            for i = 1:2:size(goal_path)
                dist_diff = sqrt(sum(goal_path(i:i+1,end)-source_path(i:i+1,end)).^2);
                if (dist_diff <= delta)
                    path = [source_path(i:i+1,:) fliplr(goal_path(i:i+1,:))];
                    hold on; plot(gca, path(1,:), path(2,:), '--b','Linewidth',1); hold on
                    total_path = [total_path; path];
%                     hold on; plot(gca, total_path(i:i+1,:), total_path(i:i+1,:), '--b','Linewidth',1); hold on
                end
                if (i == size(goal_path)-1)
                    diff = m-n;
                    for j = 1+size(goal_path):2:size(source_path);
                        dist_diff = sqrt(sum(goal_path(i:i+1,end)-source_path(j:j+1,end)).^2);
                        if (dist_diff <= delta)
                            path = [source_path(i:i+1,:) fliplr(goal_path(i:i+1,:))];
                            hold on; plot(gca, path(1,:), path(2,:), '--b','Linewidth',1); hold on
                            total_path = [total_path; path];
...
%                         hold on; plot(gca, total_path(i:i+1,:), total_path(i:i+1,:), '--b','Linewidth',1); hold on
                    ...
                    if (i == size(source_path)-1)
                        diff = n-m;
                        for j = 1+size(source_path):2:size(goal_path);
                            dist_diff = sqrt(sum(goal_path(j:j+1,end)-source_path(i:i+1,end)).^2);
                            if (dist_diff <= delta)
                                path = [source_path(i:i+1,:) fliplr(goal_path(j:j+1,:))];
                                hold on; plot(gca, path(1,:), path(2,:), '--b','Linewidth',1); hold on;
                                total_path = [total_path; path];
%                               hold on; plot(gca, total_path(i:i+1,:), total_path(i:i+1,:), '--b','Linewidth',1); hold on
                            end
...
for i = 1:2:size(total_path);
    plot(gca, total_path(1,:), total_path(1+1,:), 'k','Linewidth',1); hold on;
end
