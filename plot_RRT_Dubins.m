function plot_RRT_Dubins(gca, vertecies, edges, vert_count,vertecies1,edges1, vert_count1)

global route route1
scatter(gca, vertecies(1,1), vertecies(1,2), 45,'mp','LineWidth',3); hold on;
scatter(gca, vertecies(:,1), vertecies(:,2), 5,linspace(1,10,length(vertecies(:,1))),'filled'); hold on;
scatter(gca, vertecies1(1,1), vertecies1(1,2), 45,'gp','LineWidth',3); hold on;
scatter(gca, vertecies1(:,1), vertecies1(:,2), 5,linspace(1,10,length(vertecies1(:,1))),'filled'); hold on;

route = [];
ctr = 0;
for i=1:vert_count-1;
    ctr = ctr + 1;
    segments = 100;
    path = dubins_path_draw(edges.param(i), segments);
    plot(gca, path(1,:), path(2,:)); hold on  
    route = [route; path(:,1:102)];
end

route1 = [];
ctr = 0;
for i=1:vert_count1-1;
    ctr = ctr + 1;
    segments1 = 100;
    path1 = dubins_path_draw(edges1.param(i), segments1);
    plot(gca, path1(1,:), path1(2,:)); hold on    
    route1 = [route1; path1(:,1:102)];
end
grid on;
axis equal;
...
%     if (route(:,j) == route1(:,j))
%         display('Intersection between paths')
%     end
% end
